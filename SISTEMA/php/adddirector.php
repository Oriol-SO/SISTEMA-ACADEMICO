...
if(isset($_POST['enviar'])){
    ...
    if($nombre=='' || $apell=='' || $dni=='' || $pass==''){
        ...
        $id_direc=genera_codigo(10);
        //validar alumnos id
        $consult="SELECT*FROM DIRECTOR WHERE ID_USER='$id_direc'";
        $res=mysqli_query($con,$consult);
        ...
        while($numf>0){
            $id_direc=genera_codigo(10);
            //validar si repite o no el id
            $consult="SELECT*FROM DIRECTOR WHERE ID_DIREC='$id_direc'";
            $res=mysqli_query($con,$consult);
            $numf=mysqli_num_rows($res);
        }
        ...
        $consult2="INSERT INTO DIRECTOR(ID_USER,ID_DIREC) VALUES('$id_use','$id_direc')";
